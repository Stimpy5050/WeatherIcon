...
WeatherIcon.callback = function(data) {
	...
	c = (c > Settings.weathericon.inlineLimit ? Settings.weathericon.inlineLimit : c);
        for (i = 0; i < c; i++) {
            ...
            var icon = document.createElement('img');
            icon.className = 'weatherIcon';
            if (Settings.weathericon.useIcon) icon.src = forecast.icon;
            else icon.src = "plugins/weathericon/Icon Sets/" + Settings.weathericon.iconSet + "/" + Icons.MiniIcons[forecast.code] + Settings.weathericon.iconExt;
